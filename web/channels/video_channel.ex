defmodule KeepTalking.VideoChannel do
  use KeepTalking.Web, :channel

  def join("videos:" <> video_id, params, socket) do
    last_seen_id = params["last_seen_id"] || 0
    video_id = String.to_integer(video_id)
    video = Repo.get!(KeepTalking.Video, video_id)
    ...
    resp = %{annotations: Phoenix.View.render_many(annotations, KeepTalking.AnnotationView, "annotation.json")}
    {:ok, resp, assign(socket, :video_id, video_id)}
  end

  def handle_in(event, params, socket) do
    user = Repo.get(KeepTalking.User, socket.assigns.user_id)
  ...
  def handle_in("new_annotation", params, user, socket) do
    changeset =
      ...
      |> KeepTalking.Annotation.changeset(params)
  ...
  defp broadcast_annotation(socket, annotation) do
    ...
    rendered_ann = Phoenix.View.render(KeepTalking.AnnotationView, "annotation.json", %{
  ...
  defp compute_additional_info(ann, socket) do
    for result <- InfoSys.compute(ann.body, limit: 1, timeout: 10_000) do
      ...
      info_changeset = Repo.get_by!(KeepTalking.User, username: result.backend)
      |> build_assoc(:annotations, video_id: ann.video_id)
      |> KeepTalking.Annotation.changeset(attrs)
