defmodule KeepTalking.Channels.VideoChannelTest do
  use KeepTalking.ChannelCase
  ...
  setup do
    ...
    {:ok, socket} = connect(KeepTalking.UserSocket, %{"token" => token})
    {:ok, socket: socket, user: user, video: video}
  end

  test "join replies video annotations", %{socket: socket, video: vid} do
  ...
  test "new annotations trigger InfoSys", %{socket: socket, video: vid} do
    ...
    {:ok, _, socket} = subscribe_and_join(socket, "videos:#{vid.id}", %{})
    ref = push socket, "new_annotation", %{body: "100 * 100", at: 123}
    assert_reply ref, :ok, %{}
    assert_broadcast "new_annotation", %{body: "100 * 100", at: 123}
    assert_broadcast "new_annotation", %{body: "10000", at: 123}
  end
end
